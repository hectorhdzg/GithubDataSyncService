...
# Resolve the database path relative to this file to avoid surprises with cwd changes.
BASE_DIR = Path(__file__).resolve().parent
DEFAULT_LOCAL_DATABASE = (BASE_DIR / '..' / 'data' / 'github_issues.db').resolve()

if IS_AZURE:
    default_database_path = Path('/tmp/github_issues.db')
else:
    default_database_path = DEFAULT_LOCAL_DATABASE

DATABASE_PATH = os.environ.get('DATABASE_PATH') or str(default_database_path)

# Ensure database directory exists for both local and Azure environments
...
class GitHubSyncService:
    ...
    @staticmethod
    def _normalize_repository_identifier(repository: str) -> str:
        """Return a normalized owner/name identifier or raise ValueError."""
        if not repository:
            raise ValueError("Repository value is required")

        owner, sep, name = repository.strip().partition('/')
    ...
    def _validate_repository_on_github(self, repository: str) -> Tuple[bool, Optional[Dict[str, Any]], Optional[str]]:
        """Check if a repository exists on GitHub, returning metadata when available."""
        if requests is None:
            logger.warning("requests module unavailable; skipping GitHub validation for %s", repository)
        ...
        try:
            ...
            logger.warning("GitHub repository validation failed for %s: %s", repository, exc)
            return False, None, f"Unable to validate repository '{repository}' against GitHub: {exc}"

        if response.status_code == 200:
            try:
                return True, response.json(), None
            except ValueError:
                logger.warning("GitHub response for %s could not be parsed as JSON", repository)
                return True, None, None

        if response.status_code == 404:
            return False, None, f"Repository '{repository}' was not found on GitHub."

        if response.status_code == 403:
            return False, None, "GitHub API rate limit reached. Provide a GITHUB_TOKEN and try again."

        logger.warning(
            "GitHub validation for %s returned unexpected status %s", repository, response.status_code
        )
        return False, None, f"GitHub returned HTTP {response.status_code} while validating '{repository}'."
    ...
    def _init_database(self):
        ...
        # Backfill newer issue columns for legacy databases
        cursor.execute("PRAGMA table_info(issues)")
        issue_columns = {row[1] for row in cursor.fetchall()}
        if 'assignees' not in issue_columns:
            cursor.execute("ALTER TABLE issues ADD COLUMN assignees TEXT")
        if 'user_avatar_url' not in issue_columns:
            cursor.execute("ALTER TABLE issues ADD COLUMN user_avatar_url TEXT")
        if 'comments' not in issue_columns:
            cursor.execute("ALTER TABLE issues ADD COLUMN comments INTEGER DEFAULT 0")
        ...
        # Backfill newer pull request columns for legacy databases
        cursor.execute("PRAGMA table_info(pull_requests)")
        pr_columns = {row[1] for row in cursor.fetchall()}
        if 'user_avatar_url' not in pr_columns:
            cursor.execute("ALTER TABLE pull_requests ADD COLUMN user_avatar_url TEXT")
        if 'draft' not in pr_columns:
            cursor.execute("ALTER TABLE pull_requests ADD COLUMN draft BOOLEAN DEFAULT 0")
        if 'merged' not in pr_columns:
            cursor.execute("ALTER TABLE pull_requests ADD COLUMN merged BOOLEAN DEFAULT 0")
        if 'base_ref' not in pr_columns:
            cursor.execute("ALTER TABLE pull_requests ADD COLUMN base_ref TEXT")
        if 'head_ref' not in pr_columns:
            cursor.execute("ALTER TABLE pull_requests ADD COLUMN head_ref TEXT")
        if 'assignees' not in pr_columns:
            cursor.execute("ALTER TABLE pull_requests ADD COLUMN assignees TEXT")
        if 'comments' not in pr_columns:
            cursor.execute("ALTER TABLE pull_requests ADD COLUMN comments INTEGER DEFAULT 0")
    ...
    def get_repositories(
        ...
        cursor.execute("PRAGMA table_info(issues)")
        issues_columns = {row[1] for row in cursor.fetchall()}
        issue_repo_column = 'repo' if 'repo' in issues_columns else 'repository'

        cursor.execute("PRAGMA table_info(pull_requests)")
        pr_columns = {row[1] for row in cursor.fetchall()}
        pr_repo_column = 'repo' if 'repo' in pr_columns else 'repository'
        ...
        cursor.execute(f'''
            ...
            FROM repositories r
            LEFT JOIN (
                SELECT LOWER({issue_repo_column}) as repo_name, COUNT(*) as issue_count 
                FROM issues 
                GROUP BY {issue_repo_column}
            ) i ON LOWER(r.repo) = i.repo_name
            LEFT JOIN (
                SELECT LOWER({pr_repo_column}) as repo_name, COUNT(*) as pr_count 
                FROM pull_requests 
                GROUP BY {pr_repo_column}
            ) p ON LOWER(r.repo) = p.repo_name
            {where_clause}
        ...
        for row in raw_rows:
            ...
            row['pr_count'] = int(row.get('pr_count') or 0)

            filters_payload = row.get('filters') or '{}'
            if include_filters:
                try:
                    row['filters'] = json.loads(filters_payload)
    ...
    def add_repository(
        ...
        try:
            normalized_repo = self._normalize_repository_identifier(repo)
        ...
        try:
            ...
            cursor.execute("SELECT repo FROM repositories WHERE LOWER(repo) = LOWER(?)", (normalized_repo,))
            if cursor.fetchone():
                return {"success": False, "error": "Repository already exists"}

            is_valid, metadata, validation_error = self._validate_repository_on_github(normalized_repo)
            if not is_valid:
                return {"success": False, "error": validation_error or "Repository validation failed"}

            resolved_display_name = (display_name or '').strip()
            if not resolved_display_name:
                if metadata:
                    resolved_display_name = metadata.get('full_name') or metadata.get('name') or normalized_repo
                else:
                    resolved_display_name = normalized_repo

            resolved_classification = (classification or '').strip() or (metadata.get('language') if metadata else '') or 'Other'
            owner_login = metadata.get('owner', {}).get('login', '') if metadata else ''
            ...
            language_group = self._determine_language_group(
                normalized_repo,
            ...
            logger.info(f"Added repository: {normalized_repo}")
            return {
                "success": True,
                "message": f"Repository {normalized_repo} added successfully",
                "repository": {
                    ...
                    "is_active": bool(resolved_active)
                }
            }

        except Exception as e:
            logger.error(f"Error adding repository {normalized_repo}: {e}")
            return {"success": False, "error": str(e)}
        finally:
            if conn:
                conn.close()
...
def get_repositories():
    """Get list of repositories with optional inactive and filter metadata."""
    try:
        include_inactive = request.args.get('includeInactive', 'false').lower() == 'true'
        include_filters = request.args.get('includeFilters', 'false').lower() == 'true'
        ...
        stats = {
            "total": len(repos),
            "active": sum(1 for repo in repos if repo.get('is_active')),
            "inactive": sum(1 for repo in repos if not repo.get('is_active')),
            "issues": sum(int(repo.get('issue_count') or 0) for repo in repos),
            "pull_requests": sum(int(repo.get('pr_count') or 0) for repo in repos)
        }
